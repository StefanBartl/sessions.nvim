...
Defined in `lua/sessions/keymaps.lua`. There are no defaults for the lhs
...
All defined in `lua/sessions/usercmds.lua`; registered unconditionally by
`setup()`.

## Autocmds

`lua/sessions/autocmds.lua` wires the autosave-on-exit and (if enabled)
